...
class productClass:
    def __init__(self,root):
        ...
        product_Frame=Frame(self.root,bd=2,relief=RIDGE,bg="#fff")
        ...
        # column 1
        lbl_category=Label(product_Frame,text="Category",font=("goudy old style",18),bg="#fff").place(x=30,y=60)
        lbl_supplier=Label(product_Frame,text="Supplier",font=("goudy old style",18),bg="#fff").place(x=30,y=110)
        lbl_product=Label(product_Frame,text="Name",font=("goudy old style",18),bg="#fff").place(x=30,y=160)
        lbl_price=Label(product_Frame,text="Price",font=("goudy old style",18),bg="#fff").place(x=30,y=210)
        lbl_qty=Label(product_Frame,text="Quantity",font=("goudy old style",18),bg="#fff").place(x=30,y=260)
        lbl_status=Label(product_Frame,text="Status",font=("goudy old style",18),bg="#fff").place(x=30,y=310)
        ...
        btn_add=Button(product_Frame,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f4",fg="#fff",cursor="hand2").place(x=10,y=400,width=100,height=40)
        ...
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="#fff",cursor="hand2").place(x=340,y=400,width=100,height=40)
...
################################################################
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Product already exists , Try different ID ",parent=self.root)
                else:
    ...
    def delete(self):
        ...
        try:
            if self.var_pid.get()=="":
                messagebox.showerror("Error","Product ID is Missing !",parent=self.root)
            else:
                cur.execute("Select * from product where pid=?",(self.var_pid.get(),))
                row=cur.fetchone()
                if row==None:
